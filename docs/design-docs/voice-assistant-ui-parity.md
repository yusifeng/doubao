...
- 长按会话条目弹出会话管理菜单，支持 `编辑对话名称` 和 `从对话列表删除`。
- 删除非当前会话只更新列表，不应强制切换当前页面路由。
...
- 语音页启用 keep-awake，避免自动息屏打断沉浸式通话。
- 延续 `2003:606` 的粉紫蓝氛围背景、大头像、状态点与 transcript 胶囊。
- 切到语音模式后自动开始收听，不再要求用户二次点击“开始通话”。
- 语音模式固定为头像沉浸视图，不再提供右上角“字”按钮与文本展示切换。
- 语音模式使用三条状态文案，不再暴露 runtime 原始提示：
  - `正在听...`
  - `你已静音`
  - `说话或者点击打断`
- 底部控制条对齐豆包的四键语义：
...
- 底部不展示“静音收音/恢复收音”文案；`内容由 AI 生成` 放在四键控制条下方居中展示。
